...
if [ "$1" == "-h" ] ; then
    echo -e "Usage: "$(basename $0)" [fasta file for subset] [fasta file for extension] [reads 1] [reads 2] [# of threads to use] [read format; A, if pair end reads names end with /1 and /2, B, if not] [insert size]"
    exit 1
fi
...
r1="$(cd $(dirname "$3") && pwd -P)/$(basename "$3")" #reads file 1
r2="$(cd $(dirname "$4") && pwd -P)/$(basename "$4")" #reads file 2
threads=$5 #number of threads to use
readformat=$6 #A, if pair end reads are with /1 and /2, B, if not
isize=$7 #insert size of paired-end reads
PATH2SSAKE="/Users/carolinagandini/SSAKE_v3.8.5" #path to SSAKE folder 
...
cd subset
	...
	perl -ne 'print if ! $x{$_}++' c1_reads_mapped_titles.txt | sort -u > c1_reads_mapped_titles_uniq.txt 
...
if [ $readformat == "A" ]; then
	...
	perl -pe 's/\n/\/1\n/g'  c1_reads_mapped_titles_sorted.txt > reads_mapped_titles_1.txt
	perl -pe 's/\n/\/2\n/g'  c1_reads_mapped_titles_sorted.txt > reads_mapped_titles_2.txt
	...
	seqtk subseq $r1 c1_reads_mapped_titles_uniq.txt  | perl -pe 's/(@.*) .*/$1\/1/g' > subset4ssake_reads1.fq 
	seqtk subseq $r2 c1_reads_mapped_titles_uniq.txt  | perl -pe 's/(@.*) .*/$1\/2/g' > subset4ssake_reads2.fq
fi
