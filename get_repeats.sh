...
for f in *.fa; do ## each genome should be in indiviudal files. If a genome is composed by 2 or more chromosomes or scaffolds should be placed in a multifasta file. 
...
vsearch --cluster_size ../SR.fa --centroids SR_unique.fa --id 1 --iddef 4 --sizeout --strand both --uc clusters_unique.uc --sizeorder --minseqlength 20 --log vsearch.txt --clusters UN_CL_ 

vsearch --cluster_size SR_unique.fa --centroids centroids.fa --id 0.80 --iddef 0 --sizeout --sizein --strand both --uc clusters.uc --sizeorder --minseqlength 20 --log vsearch.txt --clusters CL_
...
awk '{if($2>=50) print $0}' cluster_abundance.txt > clusters_min50seq.txt
...
while IFS= read -r line; do
	...
	seqs=$(echo ${a[1]})
	cp "${CL}" "${CL}"_"${seqs}".fa
done < clusters_min50seq.txt

clusters_min50seq=$(cat clusters_min50seq.txt | wc -l)

if [ "$clusters_min50seq" -gt 0 ]; then
...
#get repeat bed_file with clusters names
	...
	grep ">" $f | perl -pe 's/>(.*);size=(\d*)/$1\t$2/g' > seqs.txt
	...
	size1=$(awk -v CL1=$CL1 '{if($1=="C" && $2==CL1) print $3}' clusters.uc)
			...
			while IFS= read -r line; do
				...
				cat UN_CL_"${CL2}" | grep ">" | perl -pe 's/>(.*)_(\d*)_(\d*);size=\d*/$1\t$2\t$3/g' | awk -v size1=$size1 -v CL1=$CL1 '{print $0"\t"size1"_"CL1}' | uniq >> ../SR_with_cluster.bed
			done < seqs.txt
...
seqkit fx2tab SR_repeats.fa | perl -pe 's/(.*_\d*_\d*_\d*)_(\d*)/$1\t$2/g' | awk -F '\t' -v OFS='\t' '{if($2>=50) print $1"_"$2,$3}' | seqkit tab2fx > clusters_min50seq.fa

fi

mv clusters_unique.uc vsearch_1round.uc
mv clusters.uc vsearch_2round.uc
rm seqs.txt SR_repeats.fa CL* UN_CL_*
...
echo -e "chromosomes: $(grep ">" genome.fa | wc -l)" >> repeats_stats.txt
